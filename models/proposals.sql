select
    ...
    coalesce(nullif(decidim_scopes.name->>'{{ var('DBT_LANGUAGE_CODE') }}', ''), 'Sans secteur') as decidim_scope_name,
    regexp_replace(decidim_proposals_proposals.title->>'{{ var('DBT_LANGUAGE_CODE') }}', E'(<[^>]+>)|(&[a-z]+;)', '', 'gi') as title,
    regexp_replace(decidim_proposals_proposals.body->>'{{ var('DBT_LANGUAGE_CODE') }}', E'(<[^>]+>)|(&[a-z]+;)', '', 'gi') as body,
